<?php
// 7. Write a PHP script to find the sum of numbers from 1 to N
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type="number"]:focus { border-color: #ff9a9e; outline: none; }
        input[type="submit"] {
            width: 100%; padding: 12px; background: linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%);
            color: #fff; border: none; border-radius: 8px; font-size: 16px;
            font-weight: 600; cursor: pointer; transition: opacity 0.3s;
        }
        input[type="submit"]:hover { opacity: 0.9; }
        .result {
            margin-top: 20px; padding: 20px; background: #fff0f3; border-left: 5px solid #ff9a9e;
...
<body>
    <div class="container">
        ...
        if (isset($_POST['check']) && isset($_POST['no1']) && is_numeric($_POST['no1'])) {
            ...
            echo "Sum of numbers from 1 to $n = <strong>$sum</strong>";
            echo '</div>';
        }
        ?>
    </div>
